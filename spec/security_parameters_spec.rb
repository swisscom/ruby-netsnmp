...
RSpec.describe NETSNMP::SecurityParameters do
  ...
  describe "keys" do
    ...
    let(:sha_sec) { described_class.new(security_level: :auth_priv, 
                                        ...
                                        engine_id: engine_id ) }
    it do
      expect(md5_sec.auth_key).to eq("\x52\x6f\x5e\xed\x9f\xcc\xe2\x6f\x89\x64\xc2\x93\x07\x87\xd8\x2b".b)
      expect(md5_sec.priv_key).to eq("\x52\x6f\x5e\xed\x9f\xcc\xe2\x6f\x89\x64\xc2\x93\x07\x87\xd8\x2b".b)
      expect(sha_sec.auth_key).to eq("\x66\x95\xfe\xbc\x92\x88\xe3\x62\x82\x23\x5f\xc7\x15\x1f\x12\x84\x97\xb3\x8f\x3f".b)
      expect(sha_sec.priv_key).to eq("\x66\x95\xfe\xbc\x92\x88\xe3\x62\x82\x23\x5f\xc7\x15\x1f\x12\x84\x97\xb3\x8f\x3f".b)
    end
  end
